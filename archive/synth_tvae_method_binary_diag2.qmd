...
reticulate:
  ...
  python: "/opt/homebrew/Caskroom/miniforge/base/envs/projectA/bin/python"
---
Run this once to create the project. NEVER run it twice
```{r}
# Ensure reticulate is available
if (!requireNamespace("reticulate", quietly = TRUE)) install.packages("reticulate")
library(reticulate)

# (Optional) Be explicit about which conda to use (Miniforge via Homebrew):
# reticulate::use_conda(binary = "/opt/homebrew/bin/conda")
...
if (!env_name %in% existing) {
  ...
  reticulate::conda_install(envname = env_name,
                            packages = c("pip", "numpy", "pandas", "matplotlib", "scikit-learn"),
                            pip = TRUE)
}

# Ensure R uses this env for the session (YAML already points to it; this is a safety net)
reticulate::use_condaenv(env_name, required = TRUE)

# Print config once in the render logs
reticulate::py_config()


# if I forgot a package in the list, say "seaborn"
#go to bash
#mamba activate projectA
#python -m pip install seaborn
# or within R
#library(reticulate)
#conda_install("projectA", packages = "seaborn", pip = TRUE)
# multiple packages
#conda_install("projectA", packages = c("scipy", "statsmodels"), pip = TRUE)

```



Run this each time after creation of the project
```{r}
# Load reticulate and ensure it uses the correct environment
if (!requireNamespace("reticulate", quietly = TRUE)) install.packages("reticulate")
library(reticulate)

# Use the environment defined in the YAML
reticulate::use_condaenv("projectA", required = TRUE)

# Optional: print Python config to confirm link (shows path and version)
reticulate::py_config()
```
